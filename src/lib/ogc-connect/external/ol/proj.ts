...
import {METERS_PER_UNIT, Units} from './proj/Units.js';
import {
  add as addProj,
  clear as clearProj,
  get as getProj,
...
import {Extent, applyTransform, getWidth} from './extent.js';
import {clamp, modulo} from './math.js';
import {Coordinate, equals, getWorldsAway} from './coordinate.js';
 ...
 * @api
 */
export type TransformFunction = (a:Array<number>, b?:Array<number>, n?:number) => Array<number>
 ...
 *     values).
 */
export const cloneTransform:TransformFunction = (input, output) => {
  if (output !== undefined) {
 ...
 */
export const identityTransform:TransformFunction = (input, output) =>{
...
export function getPointResolution(
  ...
  let projectionRecv = get(projection)
  if (!projectionRecv) return undefined
  projection = projectionRecv;
  let pointResolution;
  ...
  if (getter) {
    ...
    if (units && units !== projection.getUnits()) {
      ...
      if (metersPerUnit && pointResolution) {
        pointResolution =
    ...
    if ((projUnits == 'degrees' && !units) || units == 'degrees') {
      ...
      const destinationProj = get('EPSG:4326')
      let toEPSG4326
      if (destinationProj) {
      ...
      if (!toEPSG4326) return undefined
      
      if (toEPSG4326 === identityTransform && projUnits !== 'degrees') {
        // no transform is available
        pointResolution = resolution * (projection.getMetersPerUnit() ?? 1);
      } else {
        let vertices = [
 ...
 * @api
 */
export function addEquivalentProjections(projections: Array<Projection>) {
...
export function addEquivalentTransforms(
  projections1: Array<Projection>,
  projections2: Array<Projection>,
...
export function createTransformFromCoordinateTransform(
  coordTransform: (arg0: Coordinate) => Coordinate
): TransformFunction {
  return (
    /**
     ...
     */
    function (input: Array<number>, output?: Array<number>, dimension?: number): Array<number> {
...
export function addCoordinateTransforms(
  ...
  inverse: (arg0: Coordinate) => Coordinate
) {
 ...
 * @api
 */
export function getTransform(source: ProjectionLike, destination: ProjectionLike): TransformFunction | undefined {
...
export function transformWithProjections(
  point: import("./coordinate.js").Coordinate,
  sourceProjection: Projection,
  destinationProjection: Projection
): import("./coordinate.js").Coordinate {
